...
void ROUGE( int red[], int comptage [], int dim);
void VERT( int green[], int comptage [], int dim);
void BLEU( int blue[], int comptage [], int dim);
...
/*Fonctions qui permettent de récupérer les valeurs rgb d'une image*/
void ROUGE( int red[], int comptage [], int dim){
	int k;
	int i = 0;
	for(k=0; k<dim-1; k=k+3){
		red[i] = comptage[k];
		i++;
	}
}
...
int main()
{
    ...
    if (next){
        ...
        printf("0  | Image en couleur ? (y/n) ");
        scanf("%c", &reponse2);

        /*image en noir et blanc*/
        if (reponse2 != reponsePos){

             /*Récupération des dimensions de l'image*/
            for (int i=0; i<2; i++){scanf("%c", &reponse1);
...
scanf("%c", &reponse1);
            ...
            scanf("%c", &reponse1); //On met 2 scanf, sinon ça fonctionne pas (chargé de TD)
            scanf("%c", &reponse1);

            if (reponse1 == reponsePos){
                char nom[20] = {0};
    ...
    if(reponse2 == reponsePos){
        ...
        printf("après boucle 1\n");

        /*Récupère la 3è et dernière information de l'image : le niveau Max*/
        fscanf(fichier,"%d",&niveauMax);

        int comptage[niveauMax+1]; /*+1 car il y a le niveau 0.*/
        printf("comptage est cree\n");

        /*Création des tableaux de couleurs*/
        int d=dim/3;
        int red[d],green[d],blue[d];
        printf("les couleurs sont crees\n");
        ...
        ROUGE(red,comptage,dim);
        VERT(green,comptage,dim);
        BLEU(blue,comptage,dim);
        printf("%d \n",red[4]);

        /*NORMALISER LIMAGE*/
        int longueurAbs = 500;
        int occurenceMax = comptage[couleurPlusPresente(comptage, niveauMax+1)];
        int occurenceMaxR = red[couleurPlusPresente(red, d)];
        int occurenceMaxG = green[couleurPlusPresente(green, d)];
        int occurenceMaxB = blue[couleurPlusPresente(blue, d)];
        int comptageNormal[niveauMax+1];
        int redNormal[d],greenNormal[d],blueNormal[d];
        ...
        for (int i = 0; i<d + 1; i++) {
            redNormal[i] = red[i] * longueurAbs / occurenceMaxR;
            greenNormal[i] = green[i] * longueurAbs / occurenceMaxG;
            blueNormal[i] = blue[i] * longueurAbs / occurenceMaxB;
        }
